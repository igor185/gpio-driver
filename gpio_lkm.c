...
static ssize_t gpio_lkm_write (struct file *filp, const char *buf, size_t count, loff_t *f_pos)
{
    ...
    */
     ...
     */
    command_result = perform_gpio_command(gpio, which_command(kbuf));
...
static int __init gpio_lkm_init(void)
{
    ...
    {
        printk(KERN_INFO "Cannot create class %s\n", DEVICE_NAME);
    ...
    for (i = 0; i <= MAX_GPIO_NUMBER; i++)
    {
             ...
             i == 16 || i == 26)
        {
            ...
            */
            ...
            {
                printk(KERN_INFO "[GPIO_LKM]Bad kmalloc\n");
                return -ENOMEM;
            }
            ...
            {
                printk(KERN_INFO "[GPIO_LKM] - Error requesting GPIO %d\n", i);
                return -ENODEV;
            }
            ...
            */
            ...
            {
                printk (KERN_INFO "[GPIO_LKM] - Error %d adding cdev\n", ret);
...
// exported symbols
extern int perform_gpio_command (unsigned int gpio, int command) {
    struct gpio_lkm_dev *c_gpio_lkm_dev = gpio_lkm_devp[gpio];

    if (gpio > MAX_GPIO_NUMBER || !c_gpio_lkm_dev) {
        printk("[GPIO_LKM] - Cannot command GPIO %d, it is not availabe on the device\n", gpio);
        return -EPERM;
    }
...
EXPORT_SYMBOL(perform_gpio_command);
...
EXPORT_SYMBOL(read_gpio_value);

extern int gpio_lkm_free_gpio(unsigned int gpio) {
    if (gpio_lkm_devp[gpio]) {
        gpio_direction_output(gpio, 0);
        device_destroy (gpio_lkm_class, MKDEV(MAJOR(first), MINOR(first) + gpio));
        gpio_free(gpio);

        gpio_lkm_devp[gpio] = 0;
        return 0;
    }
    return 0;
}

EXPORT_SYMBOL(gpio_lkm_free_gpio);